...
def measurement_by_sensor_from_to():
    ...
    if not sensor_id or not time_from or not time_to:
        return jsonify({"error": "sensor_id, time_from and time_to are required"}), 400
    ...
    try:
        ...
        return jsonify(
            {"error": f"Datetime format must be {DATETIME_FORMAT}"}
        ), 400

    sensor_info = get_measurement_by_sensor_from_to(
